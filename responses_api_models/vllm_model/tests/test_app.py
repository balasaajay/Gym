...
class TestTopLogprobsHandling:
    ...
    def test_capture_path_uses_dynamo_logprobs_without_consuming_choice_logprobs(self) -> None:
        model = _make_top_logprobs_model(return_token_id_information=True)
        app = model.setup_webserver()
        choice_logprobs = {
            "content": [
        ...
        assert data["choices"][0]["logprobs"]["content"] == choice_logprobs["content"]
        mock_client.create_tokenize.assert_not_awaited()

    @mark.parametrize(
        ("engine_data", "error"),
        [
            ({"completion_token_ids": [123], "completion_logprobs": [-0.1]}, "partial token metadata"),
            ([], "must be an object"),
            (
                {
                    "prompt_token_ids": [10, 20],
                ...
                "mismatched generation token IDs",
            ),
        ],
    )
